(ns vector-ops.benchmarks.bench
...
(defn output-file [name] (str "./benchmarks/data/" name "-benchmark.csv"))

(defn extract-estimate [result]
  (or (re-find #"[0-9]+.[0-9]+ s" result)
      (re-find #"[0-9]+.[0-9]+ ms" result)
      (re-find #"[0-9]+.[0-9]+ ns" result)
      (re-find #"[0-9]+.[0-9]+ .s" result)))
...
(defn bench-take-last-clj []
  (run-benchmark {:name      "clj-take-last"
                  :ranges    data-ranges
                  :data-type :half-val
                  :expr      (fn [data n] (vec (take-last n data)))}))

(defn bench-take-last-opt []
  (run-benchmark {:name      "opt-take-last"
                  :ranges    data-ranges
                  :data-type :half-val
                  :expr      (fn [data n] (o/takev-last n data))}))

(defn bench-drop-last-clj []
  (run-benchmark {:name      "clj-drop-last"
                  :ranges    data-ranges
                  :data-type :half-val
                  :expr      (fn [data n] (vec (drop-last n data)))}))

(defn bench-drop-last-opt []
  (run-benchmark {:name      "opt-drop-last"
                  :ranges    data-ranges
                  :data-type :half-val
                  :expr      (fn [data n] (o/dropv-last n data))}))
...
(def benchmarks
  {:clj-concat          bench-concat-clj
   ...
   :clj-take-last       bench-take-last-clj
   :opt-take-last       bench-take-last-opt
   :clj-drop-last       bench-drop-last-clj
   :opt-drop-last       bench-drop-last-opt
